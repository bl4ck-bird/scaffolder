...
fn apply_with_docker_and_ci_stacks_renders_conditional_files_and_dedups_gitignore() {
    ...
    assert!(target.join("Cargo.toml").exists(), "Cargo.toml must be written");
    assert!(target.join("README.md").exists(), "README.md must be written");
    assert!(target.join("src/main.rs").exists(), "src/main.rs must be written");
    assert!(target.join(".gitignore").exists(), ".gitignore must be written");
    assert!(target.join(".editorconfig").exists(), ".editorconfig must be written");
    assert!(target.join("scripts/build.sh").exists(), "scripts/build.sh must be written");

    let cargo_toml = fs::read_to_string(target.join("Cargo.toml")).expect("read Cargo.toml");
    assert!(cargo_toml.contains("name = \"demo\""), "Cargo.toml must render project name");

    let main_rs = fs::read_to_string(target.join("src/main.rs")).expect("read src/main.rs");
    assert!(main_rs.contains("Hello, world!"), "src/main.rs must be copied verbatim");
    ...
    assert!(gitignore.contains("/target"), ".gitignore must include the rust base pattern");
    assert!(gitignore.contains(".env"), ".gitignore must include the docker fragment");
    assert_eq!(
    ...
    assert!(target.join("Dockerfile").exists(), "Dockerfile must be written when docker is selected");
    assert!(
    ...
    assert!(build_sh.contains("docker build"), "build.sh must render the docker-conditional block");
    ...
    #[cfg(unix)]
    {
        ...
        let mode = fs::metadata(target.join("scripts/build.sh"))
            ...
            .permissions()
            .mode();
        assert_ne!(mode & 0o111, 0, "scripts/build.sh must have an execute bit set");
    }
}
...
fn apply_with_docker_only_stack_renders_dockerfile_without_ci() {
    ...
    assert!(target.join("Dockerfile").exists(), "Dockerfile must be written when docker is selected alone");
    assert!(
...
fn apply_with_no_stacks_skips_conditional_files_and_excludes_docker_fragment() {
    ...
    assert!(target.join("Cargo.toml").exists(), "Cargo.toml must still be written");
    assert!(
        !target.join("Dockerfile").exists(),
    ...
    assert!(gitignore.contains("/target"), ".gitignore must still include the rust base pattern");
}
