...
/// 훅 실행·overwrite·외부쓰기 confirm 게이트(§1.9-5, §1.10). infra가 대화형으로 구현한다.
pub trait Confirmer {
    /// 훅 실행 전 confirm. `description`은 인라인 명령 또는 `run <file>` 표시(§1.9-5).
    ...
    /// target 밖 쓰기 confirm(payload 외부 심링크 포함, §1.10).
